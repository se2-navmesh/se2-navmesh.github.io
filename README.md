...
Static widgets in `static/js/interactives.js` (vanilla JS, no build step): NavMesh
vs. SE(2) comparison slider, yaw-feasibility demo, ASA stepper, and the SPC
benchmark chart. MathJax and Chart.js load from CDN; Three.js
(+ addons) loads from the import map in `index.html`.

## Videos

The Videos section embeds one YouTube video panel:
`https://www.youtube.com/embed/lUle0EkAsEw`.
